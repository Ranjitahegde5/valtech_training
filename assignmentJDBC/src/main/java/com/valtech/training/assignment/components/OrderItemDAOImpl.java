...
//	int orderId, int quantity, int productId, double amount
	private String insertQuery=
			"INSERT INTO ORDER_ITEMS(ITEM_ID,ORDER_ID,QUANTITY,PRODUCT_ID,AMOUNT) VALUES(?,?,?,?,?)";
	private String countQuery="SELECT COUNT(*) FROM ORDER_ITEMS ";
	private String updateQury="UPDATE ORDER_ITEMS SET ITEM_ID=?,ORDER_ID=?,QUANTITY=?,"
	...
	public void addOrderItems(OrderItems items) {
		...
		new JdbcTemplate(dataSource).update(insertQuery,items.getItemId(), items.getOrderId(),items.getQuantity(),
